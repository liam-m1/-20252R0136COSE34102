...
kfree(char *v)
{
  ...
  int refCount = get_refcount(V2P(v));

  // if process is allocated, decrease reference count
  if(refCount > 0) {
    dec_refcount(V2P(v));
    refCount--;
  }

  // if process has no more references, free
  if(refCount == 0) {
...
kalloc(void)
{
  ...
  pmem.num_free_pages--;
  // when allocated, increase reference counter
  inc_refcount(V2P(r));
